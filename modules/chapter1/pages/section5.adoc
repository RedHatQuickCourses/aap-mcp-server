...
==== 3.1.4: Troubleshooting OpenCode MCP Integration
...
=== 3.2: Using LibreChat with the Ansible MCP Server
...
+
image::librechat-edit-secret.png[Edit the librechat-credentials-env Secret,link=self,window=blank]
+
...
+
image::librechat-edit-secret-value.png[Edit ANSIBLE_MCP_TOKEN in the librechat-credentials-env Secret,link=self,window=blank]
+
...
+
image::librechat-pods-filter.png[Filter the librechat Pods by Label,link=self,window=blank]
+
...
+
image::librechat-delete-pod.png[Delete the librechat Pod,link=self,window=blank]
+
. Wait until the pod has been recreated, is _Running_, and displays `1/1` in the _Ready_ column.
+
. Launch the LibreChat web interface by navigating to: link:https://librechat-librechat.{openshift_cluster_ingress_domain}[LibreChat Web Interface,window=_blank]. You will be presented with the LibreChat login page.
+
image::librechat-login-page.png[LibreChat Login Page,link=self,window=blank]
+
...
+
image::librechat-enable-model.png[Enable the {litellm_available_models_list} Model in LibreChat,link=self,window=blank]
+
...
+
image::librechat-enable-mcp.png[Enable aap MCP Server in LibreChat,link=self,window=blank]
+
Now, lets start a chat session using our natural language that retrieves details that originate from Ansible Automation Platform that is facilitated through the integration with the AAP MCP Server.
+
...
image::librechat-chat-result.png[LibreChat Chat Result,link=self,window=blank]
